...
## check the specific species of top 20 decreased and increased region
top20_inc_mammal = (Delta_ED_mammal %>% arrange(desc(delta_mean_ED)) %>% pull(RegionID))[1:20]
top20_dec_mammal = (Delta_ED_mammal %>% arrange(delta_mean_ED) %>% pull(RegionID))[1:20]
...
ED_natives_mammal = ED_mammal_native$ED_list
names(ED_natives_mammal) = sort(unique(sp_dis_5$Region.ID))

top20_inc_ED_mammal_exotic = comm_mammal_exotic %>% 
  filter(RegionID %in% top20_inc_mammal & presence == 1) %>% 
  left_join((df %>% sf::st_drop_geometry())[,c('Level_4_Na', 'RegionID')], 
            by = 'RegionID')

top20_inc_ED_mammal_native = ED_natives_mammal[which(names(ED_natives_mammal) %in% top20_inc_mammal)] 

top20_dec_ED_mammal_exotic = comm_mammal_exotic %>% 
  filter(RegionID %in% top20_dec_mammal & presence == 1) %>% 
  left_join((df %>% sf::st_drop_geometry())[,c('Level_4_Na', 'RegionID')], 
            by = 'RegionID')

top20_dec_ED_mammal_native = ED_natives_mammal[which(names(ED_natives_mammal) %in% top20_dec_mammal)] 
...
Delta_ED_all_taxa = rbind(Delta_ED_mammal[,colnames(Delta_ED_plant)], Delta_ED_plant, 
...
Delta_ED_all_taxa$f_ini_div_t = factor(Delta_ED_all_taxa$ini_div_t)
...
colnames(Delta_ED_all_taxa)

Delta_ED_all_taxa$ini_div_t = as.numeric(Delta_ED_all_taxa$ini_div_t)
mod_inv_impact_time = lm(delta_mean_ED ~ ini_div_t,
                            data = Delta_ED_all_taxa
)
summary(mod_inv_impact_time)

plot_inv_impact_time = sjPlot::plot_model(mod_inv_impact_time,
                                          type = 'pred', 
                                          show.data = T)
