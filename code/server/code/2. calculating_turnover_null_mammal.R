...
sp_overlap_dat_2_num = sp_overlap_dat_2 %>% group_by(RegionID) %>% 
  summarise_at(vars(Binomial), length) %>% 
  complete(RegionID = df_trans$RegionID, fill = list(Binomial = 0)) %>%
  arrange(Binomial) 

top10per_posi = round(length(unique(sp_overlap_dat_2_num$Binomial)) * 0.1)
top10per = sort(unique(sp_overlap_dat_2_num$Binomial))[1:top10per_posi]
top10per_regions = sp_overlap_dat_2_num %>% filter(Binomial %in% top10per) %>%
                   pull(RegionID)
...
region_pool_l = lapply(top10per_regions, function(region_i) {
  ...
  neighbor_mammal_exotic = sp_mammal_all %>% 
  ...
  focal_mammal_exotic = sp_mammal_all %>% 
  ...
  neighbor_mammal_native = sp_mammal_all %>% 
  ...
  candidate_species = neighbor_mammal_exotic %>%
    setdiff(neighbor_mammal_native) %>%
    setdiff(focal_mammal_exotic)
  ...
  candidate_dat = sp_mammal_all %>% 
    filter(RegionID %in% neighbor_regions_2,
           Binomial %in% candidate_species)
...
exotic_mammal_null_l = vector("list", 1000)
...
for (r in seq_len(1000)) {
  ...
  add_l = lapply(region_pool_l, function(pool) {
    ...
    select_mammal_exotic = sample(
      pool$candidate_species,
      size = pool$num
    )
    
    # Extract corresponding records
    select_mammal_exotic_dat = pool$candidate_dat %>% 
      filter(Binomial %in% select_mammal_exotic) %>% 
      slice_head(n = pool$num)
    
    # Relocate selected species to the focal region
    select_mammal_exotic_dat$RegionID = pool$region_i
    
    select_mammal_exotic_dat
  })
  ...
  exotic_mammal_null_l[[r]] = sp_overlap_dat_null
}


save(exotic_mammal_null_l,
     file = 'results/primary_results/null_models/exotic_mammal_null_l.rdata')

## d. calculating phylogenentic Simpson similarity for constructed null communities
load("results/primary_results/null_models/exotic_mammal_null_l.rdata")

## Precompute fixed region and species sets
# All species appearing in the observed and null datasets
all_species = unique(c(sp_dis_6$Binomial,
  unlist(lapply(exotic_mammal_null_l, function(x) unique(x$Binomial)))))
all_species = intersect(all_species, spec_phy.3$tip.label)

# All regions appearing in the observed and null datasets
all_regions = unique(c(sp_dis_6$RegionID,
  unlist(lapply(exotic_mammal_null_l, function(x) unique(x$RegionID)))))
...
phylo_mammal_extant_fixed = drop.tip(spec_phy.3,
 setdiff(spec_phy.3$tip.label, all_species))

# Use the tree tip order as the matrix column order
species_order = phylo_mammal_extant_fixed$tip.label

## 3. Construct the baseline community matrix once
sp_dis_6_base = sp_dis_6 %>%
  filter(Binomial %in% species_order) %>%
...
idx_region = match(as.numeric(sp_dis_6_base$RegionID), all_regions)
idx_species = match(sp_dis_6_base$Binomial, species_order)

native_mat[cbind(idx_region, idx_species)] = sp_dis_6_base$presence
storage.mode(native_mat) = "numeric"

plan(multisession, workers = round(parallel::detectCores()*0.5))

system.time({
phy_uni_mammal_extant_null = future_map(
  exotic_mammal_null_l,
  function(x) {
    ...
    mat_mammal_extant = native_mat
    
    idx_region = match(as.numeric(x2$RegionID), all_regions)
    idx_species = match(x2$Binomial, species_order)
    
    mat_mammal_extant[cbind(idx_region, idx_species)] = pmax(
      mat_mammal_extant[cbind(idx_region, idx_species)],
      x2$presence
    )
    
    calcu_phy_sim_multiple_all(
      tree = phylo_mammal_extant_fixed,
      x = mat_mammal_extant,
      block_size = 40000
    )
  },
...
save(phy_uni_mammal_extant_null,
     file = 'results/primary_results/null_models/phy_uni_mammal_extant_null.rdata')



#3. Calculating the delta_PU for the 1000 null models that relocates exotic species from neighboring regions ----
load('results/primary_results/null_models/phy_uni_mammal_extant_null.rdata')
load("results/primary_results/distances_beta/phy_turn_mammal_native.rdata")
turn_mammal_native = data.frame(RegionID = rownames(phy_turn_mammal_native),
                               turn = rowMeans(phy_turn_mammal_native))
turn_mammal_native$RegionID = as.integer(turn_mammal_native$RegionID)

system.time({
  phy_turn_mammal_delta_null = future_map(
    phy_uni_mammal_extant_null,
    function(x) {
      #x = phy_uni_mammal_extant_null[[1]]
      phy_turn_mammal_all = x$simpson
      turn_mammal_all = data.frame(RegionID = rownames(phy_turn_mammal_all),
                                      turn = rowMeans(phy_turn_mammal_all))
      turn_mammal_all$RegionID = as.integer(turn_mammal_all$RegionID)
      
      colnames(turn_mammal_all)[which(colnames(turn_mammal_all) == 'turn')] = 'all_turn'
      PU_mammal_delta = turn_mammal_native %>% 
        left_join(turn_mammal_all, by = 'RegionID')
      PU_mammal_delta$delta_PU = log(PU_mammal_delta$all_turn/
                                       PU_mammal_delta$turn)
      return(PU_mammal_delta)
...
phy_turn_mammal_delta_null_dat = data.table::rbindlist(phy_turn_mammal_delta_null)

phy_turn_mammal_delta_null_dat$rep = rep(c(1:1000),
                                         each = length(unique(phy_turn_mammal_delta_null_dat$RegionID)))

phy_turn_mammal_delta_null_summary = phy_turn_mammal_delta_null_dat %>% group_by(RegionID) %>% 
...
hist(phy_turn_mammal_delta_null_summary$Q1)
hist(phy_turn_mammal_delta_null_summary$median)
hist(phy_turn_mammal_delta_null_summary$Q3)

save(phy_turn_mammal_delta_null_summary,
     file = 'results/primary_results/null_models/phy_turn_mammal_delta_null_summary.rdata')
